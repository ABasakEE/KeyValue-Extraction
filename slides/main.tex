...
\begin{frame}[plain]
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      {\bfseries Team: \alert{TEAM NAME TBD}}\\[0.25cm]
      \begin{tabular}{@{}ll@{}}
...
\begin{frame}{Family 2 — Decoupling and Relative Encoding}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\small
        \item Two streams: a swappable text tower and a layout tower.
        \item Coupled at every layer by \textbf{BiACM}; the layout$\to$text gradient is \textbf{detached} during pretraining.
        \item Rationale: layout is language-agnostic, so do not let it entangle with one vocabulary.
        \item FUNSD SER F1 \textbf{88.41}. Pretrained on English, transfers to 7 unseen languages on XFUND.
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{BROS} \hfill {\footnotesize Apache-2.0}
      \begin{itemize}\small
        \item \textbf{No visual features at all.}
        \item Relative 2D encoding: attention bias from the offset between box pairs.
        \item \textbf{Area-masked LM} — masks contiguous 2D regions, forcing layout use.
        \item SPADE-style graph decoder for linking, avoiding reading-order dependence.
...
\begin{frame}{Family 3 — Structure Beyond Sequences: Graphs and Parsing}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
      \small
      \textbf{The idea.} Linking is \emph{relational}. Model the document as a graph:
      \textbf{nodes} are text regions (tagging = node classification), \textbf{edges} are
      candidate key-value relations (linking = \textbf{edge} classification). No reading
      order assumed.
      \smallskip
    ...
    \begin{column}{0.47\textwidth}
      ...
      {\footnotesize\color{warn}\textbf{Shared weakness.} The graph is built \emph{before} the GNN runs, so bad OCR boxes corrupt everything downstream — a failure mode sequence taggers do not have in the same form.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{What Is Actually \emph{Measured} About Template Shift}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{alertblock}{\small Design consequence}
...
\begin{frame}{Dataset Survey}
  ...
  \begin{alertblock}{The structural problem}
    \small The dataset with an official unseen-template protocol (\textbf{VRDU}) has \textbf{no
...
\begin{frame}{Metrics, Failure Taxonomy and Early Diagnostics}
  \begin{columns}[T]
    \begin{column}{0.33\textwidth}
      \textbf{Metrics}
      \begin{itemize}\footnotesize
        \item \textbf{Entity F1} — strict IOB2, exact span \emph{and} type, via \texttt{seqeval}.
        \item \textbf{Linking F1} — a (key, value) pair counts only on exact match; strictly harder, since both endpoints must already be right.
        \item \textbf{Gap} — MTL minus UTL, matched size, $\geq$3 seeds.
      \end{itemize}
      {\scriptsize\color{muted} Implemented twice, independently, and cross-checked — which caught a strict-vs-lenient IOB2 mismatch worth 0.66 F1.}
    \end{column}
    \begin{column}{0.33\textwidth}
      \textbf{Four failure buckets}
      \begin{enumerate}\footnotesize
        \item \textbf{OCR / recognition} — wrong characters
        \item \textbf{Layout association} — right text, wrong key
        \item \textbf{Field type} — right place, wrong label
        \item \textbf{Repeated structure} — line items split or merged
      \end{enumerate}
      {\scriptsize The entity-F1 minus linking-F1 difference separates bucket 1 from bucket 2.}
    \end{column}
    \begin{column}{0.33\textwidth}
      \textbf{Diagnostics instrumented}
      \begin{itemize}\footnotesize
        \item \textbf{Malformed-tag rate} — illegal \texttt{I-} chunk openings. A rise on unseen templates is direct evidence the model is \emph{guessing} at boundaries.
        \item \textbf{Strict/lenient spread} — widens as tag sequences degrade.
        \item \textbf{Truncation count} — words past the 512-token cap, scored as absent, never as \texttt{O}.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Experiment 1 — Pipeline Validation (FUNSD)}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{Setup.} LiLT (\texttt{lilt-roberta-en-base}, MIT), FUNSD standard 149/50 split,
      3 seeds, best-validation checkpoint. Validation is carved out of train so the test set is
      never touched during model selection.
      \medskip

      \textbf{Purpose.} Confirm that tokenisation, box normalisation (0--1000),
      word$\to$subword label alignment and the metric are all correct \emph{before} any
      cross-template number is trusted.
      \medskip

      \begin{block}{Result}
        \centering\large
        Entity F1 = \alert{[PENDING]}\\
        {\small target $\approx$ 0.88}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Read this number correctly}
        \small
        This is a \textbf{code-path validation}, not a generalization result. FUNSD's train and
        test sets share 16\% of their templates, and its block-level annotation leaks entity
        boundaries. A high score here says our pipeline works — and nothing more.
      \end{alertblock}
      \medskip
      {\footnotesize\color{muted}\textit{[Training-curve figure to be inserted: \texttt{figures/funsd\_curve.pdf}]}}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Experiment 2 — Seen vs.\ Unseen Templates (VRDU)}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{Setup.} VRDU Registration Forms, leave-one-template-out over 3 templates, MTL and
      UTL at \emph{matched} training-set size, 3 seeds each. The leakage assertion passes on
      every fold.
      \medskip
      ...
      \begin{block}{Result}
        ...
        \begin{tabular}{@{}lr@{}}
          \toprule
          Seen (MTL), mean F1 & \alert{[PENDING]}\\
          Unseen (UTL), mean F1 & \alert{[PENDING]}\\
          \midrule
          \textbf{Gap} & \alert{[PENDING]}\\
          \bottomrule
        \end{tabular}
      \end{block}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Calibration.} VRDU reports 13--17 points across models; FormNet at 200 training
      documents shows 90.51 $\to$ 77.29, a \gap{13.22-point} gap.
      \medskip

      {\footnotesize\color{muted}\textit{[Per-template bar chart: \texttt{figures/vrdu\_gap.pdf}]}}
      \medskip

      \textbf{Hypothesis under test.} Errors on unseen templates will concentrate in
      \textbf{bucket 2} (layout association: right text, wrong key) rather than bucket 1
      (recognition) — because OCR quality does not change when the layout does.
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{The Research Gap We Are Addressing}
  \begin{alertblock}{Observation from our review}
    Entity extraction \emph{has} been measured under template shift — VRDU, DocILE and Do-GOOD
    all do it. Key-value \textbf{linking} has not. Every strong linking number we found comes
    from a split where train and test share templates.
  \end{alertblock}

  \medskip
  \small
  \begin{tabular}{@{}lp{4.2cm}p{3.7cm}@{}}
    \toprule
  ...
  \medskip
  \centering\textbf{Our contribution: measure tagging \emph{and} linking under one template-disjoint protocol.}\\
  {\footnotesize\color{muted} Stated as "to our knowledge" — we have not exhaustively verified DocILE's task structure.}
\end{frame}

% ======================== 21. ROADMAP ================================
\begin{frame}{Subtasks and Roadmap}
  \small
  \begin{tabular}{@{}clp{5.2cm}c@{}}
    ...
    1 & S1 VRDU loader & Parse VRDU to words/boxes/tags with template ids & PM\\
    2 & S2 Gap measurement & MTL vs UTL, matched size, $\geq$3 seeds & PM\\
    ...
    6 & S6 Ablations & absolute vs relative position; $\pm$vision; OCR-free; reading-order perturbation; $k$-NN attention & PM\\
    7 & S7 Failure analysis & Per-template, per-bucket error breakdown & both\\
...
\begin{frame}{References — Architectures}
  \tiny
  \begin{enumerate}\setcounter{enumi}{11}\itemsep1.5pt
...
\begin{frame}{References — Generalization, Tooling and Websites}
  \tiny
  \begin{enumerate}\setcounter{enumi}{26}\itemsep1.5pt
  ...
  \smallskip
  \textbf{Websites}
  \begin{enumerate}\setcounter{enumi}{35}\itemsep1.5pt
