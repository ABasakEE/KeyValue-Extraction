  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \begin{alertblock}{Read this number correctly}
        \scriptsize
        \textbf{Code-path validation}, not generalization. FUNSD shares 16\% templates and leaks block boundaries; this confirms pipeline correctness.
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Experiment 2 — Seen vs.\ Unseen Templates (VRDU)}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      {\footnotesize\textbf{Setup.} VRDU Registration Forms, leave-one-template-out (3 folds $\times$ 3 seeds = 18 runs), matched size ($n=200$). Leakage check: 0 test templates in train.}
      \vspace{0.2cm}

      \begin{block}{Result (Matched $n=200$, 9 folds each)}
        \scriptsize
        \begin{tabular}{@{}lr@{}}
          \toprule
    ...
    \begin{column}{0.52\textwidth}
      ...
      \textbf{Calibration:} FormNet gap is 13.2 pts ($90.5 \to 77.3$); LiLT drops \gap{21.1 pts}, confirming vulnerability to unseen layouts.}
    \end{column}
  \end{columns}
\end{frame}
